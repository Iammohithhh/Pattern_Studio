...
export class ClassroomRenderer {
 ...
 setFrame(frame){
  ...
  if(!this.initialized){const distance=Math.max(12,frame.extent*1.25,frame.extent/(2*Math.tan(19*Math.PI/180)*this.camera.aspect));this.camera.position.set(distance*.22,distance*.65,distance);this.controls.target.set(0,.4,0);this.controls.update();this.controls.saveState();this.initialized=true;}
 }
