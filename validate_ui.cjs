...
for(const name of ['curriculum','neetcode','topics','app','stories','cinema-model','cinema-ui','studio'])vm.runInContext(fs.readFileSync(`dist/${name}.js`,'utf8'),context,{filename:name+'.js'});
vm.runInContext(`
...
for(const p of data){
 ...
 for(let i=0;i<p.trace.length;i++){step=i;visual(p);const f=CinemaModel.frame(p,i);if(!f.objects.length||!f.code||!f.explanation)throw Error('Incomplete 3D frame '+p.id+':'+i);const ids=new Set();for(const o of f.objects){if(ids.has(o.id)||![o.x,o.y,o.z].every(Number.isFinite))throw Error('Invalid 3D object '+p.id+':'+i);ids.add(o.id);}if(f.final!==(i===p.trace.length-1))throw Error('Wrong final checkpoint');snapshots++;}
}
